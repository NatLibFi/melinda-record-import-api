...
module.exports.ensureAuthenticated = function (req, res, next) {
	if (process.env.REQ_AUTH === 'false') { // Authentication can be skipped via environment variable REQ_AUTH = false
		Logger.log('debug', `REQ_AUTH set to ${process.env.REQ_AUTH} -> Skipping authentication`);
	...
	getUsernamePromise.then(username => {
		...
		getProfilenamesPromise.then(profileName => {
			Logger.log('debug', `Trying to use profile: ${profileName}`);
			...
			if (profileName) { // User is trying to access some profile
				...
				getAuthenticationGroupsPromise.then(authGroups => {
					Logger.log('debug', `Authentication groups: ${authGroups}`);

					const isUserInGroupsPromise = isUserInGroups(username, authGroups);
					isUserInGroupsPromise.then(isInGroup => {
						if (isInGroup) {
							Logger.log('debug', 'User can continue to Endpoint');
							return next(); // User can continue to EP
						}

						Logger.log('debug', 'User is in not in any usergroup they are trying to use, they cannot continue to Endpoint');
				...
				Logger.log('debug', 'User is not trying to use any profile, they can continue to Endpoint');
				return next(); // User can continue to EP
			}
		}).catch(err => {
			return next(err);
	...
	return new Promise((resolve, reject) => {
		...
		function callback(error, response, body) {
			if (!error && response.statusCode === 200 && body) {
				...
				Logger.log('debug', 'Not able to authenticate user with basic auth, throwing unauthorized error');
				reject(serverErrors.getUnauthorizedError());
			}
		}
	...
	return new Promise((resolve, reject) => {
		...
		function callback(error, response, body) {
			if (!error && response.statusCode === 200 && body.user) {
				...
				Logger.log('debug', 'Not able to authenticate user with SSO, throwing unauthorized error');
				reject(serverErrors.getUnauthorizedError());
			}
		}
...
function getProfilename(req) {
	...
	return new Promise((resolve, reject) => {
		...
		Logger.log('debug', `Profilename: ${profileName}, blobID: ${blobID}`);
